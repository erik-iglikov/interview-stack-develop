...
3. api.prodcuts -> Backend for the products functionality (written in flask)
...
To build the docker image for api.prodcuts run the command bellow from the root of the `api.prodcuts` directory

```Bash
docker build -t api.prodcuts:latest .
```
...
To submit the solution please email `aadeyeye@marzvfx.com` with a link to your github repo with the implemented task. For the email subject please specify your full name (first and last name) as well as the role you are applying for
